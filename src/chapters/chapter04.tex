...
\input{figures/chapter04_diagram01}
...
\todo{Can we point to the code somewhere on github and add as reference/citation}
...
\input{figures/chapter04_diagram03}
...
\input{figures/chapter04_diagram02}
...
You're working with SafeTensors. The format stores weights at their original precision (typically FP16 or BF16). For quantized inference, you'll either apply quantization at load time (using BitsAndBytes or AutoGPTQ) or download a pre-quantized version in GGUF format.

\todo{Example/Sample code for loading/using a SafeGTensor model}
...
\item Running on Apple Silicon (Metal support)
\end{itemize}
...
The Protobuf encoding trades mmap efficiency for flexibility and cross-platform compatibility. This matters less for smaller models but can impact loading times for 7B+ parameter models.
...
The following table summarizes how each format implements our design principles:
...
As of late 2024, several model families dominate the open-weight landscape:
...
Hugging Face Hub is the central repository for open models~\cite{huggingface2024}. With over 500,000 models (as of late 2024), navigation skills are essential.
...
\begin{itemize}
\item \textbf{One-command deployment}: \texttt{ollama run llama3.2:7b}
\item \textbf{Automatic quantization selection}: Downloads appropriate variant for your hardware
\item \textbf{Version management}: \texttt{ollama pull llama3.2:7b-q4\_K\_M}
