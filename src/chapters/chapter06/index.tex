...
\item[Observability] Every request is measured. Latency histograms, error rates, token throughput---all available in Prometheus and Grafana.
...
\item \textbf{Prometheus Metrics}: Request count, latency histogram, error rate
...
\item[Metrics Collector] Records request metrics (count, latency, tokens). Exposes Prometheus endpoint at \texttt{/metrics}.
...
You can't improve what you can't measure. v0.1 includes observability from day one.
...
\begin{description}
\item[Request Volume] \texttt{inference\_requests\_total} --- Counter of requests by model and status (success/error). Answers: How busy are we? What's the error rate?

\item[Latency] \texttt{inference\_request\_duration\_seconds} --- Histogram of request duration. Answers: What's the p50/p95/p99 latency? Are we meeting SLOs?

\item[Throughput] \texttt{inference\_tokens\_generated\_total} --- Counter of tokens generated. Answers: What's our actual output? (More meaningful than request count for LLMs.)

\item[Backend Health] \texttt{inference\_backend\_healthy} --- Gauge (1 = healthy, 0 = unhealthy). Answers: Is our backend up? (Useful for alerting.)
\end{description}
...
\begin{itemize}
\item \texttt{request\_id}: UUID for tracing across components
...
JSON-formatted logs can be shipped to any log aggregator (Elasticsearch, Loki, CloudWatch).
...
The control plane is the only service your application needs to know about. Prometheus scrapes metrics from the control plane every 15 seconds. Grafana queries Prometheus to render dashboards.
...
\item \textbf{Prometheus}: 512MB RAM (grows with metric cardinality)
\item \textbf{Grafana}: 256MB RAM
\end{itemize}
...
\subsection{Viewing Metrics}
\label{subsec:viewing-metrics}

After making some requests, check the metrics:
...
The repository includes a pre-configured Grafana dashboard showing request rate, latency percentiles, and error rate.
...
\item Observability (metrics, logging, health checks) is built in from day one
