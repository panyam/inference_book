...
{
  "model": "qwen2.5:7b",
  "response": "AI inference is the process of using a trained...",
  "done": true,
  "total_duration": 1284000000,
...
In the response above, \texttt{prompt\_eval\_count: 12} indicates your prompt consumed 12 tokens. The phrase "Explain what AI inference is in one paragraph." contains 10 words but 12 tokens---the tokenizer made different choices than word boundaries. The field \texttt{eval\_count: 87} shows the model generated 87 tokens in response.
...
\begin{itemize}
\item \texttt{total\_duration}: Wall-clock time for the entire request---1.28 seconds in our example.
\item \texttt{eval\_duration}: Time spent generating tokens---1.15 seconds. The difference (130ms) covers prompt processing and overhead.
\end{itemize}

From these numbers, you can calculate \emph{tokens per second}: $87 \text{ tokens} / 1.15 \text{ seconds} \approx 76 \text{ tok/s}$. This metric---generation throughput---is the primary performance indicator for inference systems. Higher is better; it directly affects how fast users see responses.
...
\item[Inter-Token Latency (ITL)] Average time between generated tokens. With 87 tokens in 1.15 seconds, ITL is approximately 13ms. Lower ITL means smoother streaming.
\end{description}
