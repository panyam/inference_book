...
\calclink{/c/3/vram}{}{Try the interactive Weight Memory calculator}
...
Quantization reduces precision further. At 4-bit precision (INT4), each parameter uses only 0.5 bytes, bringing our 7B model down to 3.5 GB. This is why quantized models run on laptops and modest GPUs---the memory reduction is dramatic. Section~\ref{subsec:precision} covers precision formats in detail, and Chapter~\ref{ch:models} explores quantization methods.
...
The KV cache is the second major memory consumer, and unlike weights, it grows during inference. As we learned in Chapter~\ref{ch:how-llms-work}, the KV cache trades memory for compute by storing key and value vectors from previous tokens. Here we focus on quantifying that memory cost.
...
This is why long-context inference is memory-intensive. A 7B model that fits comfortably in 8 GB VRAM at 4K context may require 24 GB at 32K context. Grouped Query Attention (GQA), used in newer models like Llama 3 and Qwen 2.5, reduces KV cache size by sharing key-value heads, typically by 4--8x. When selecting models in Chapter~\ref{ch:models}, we'll see how GQA affects practical deployment choices.
...
\calclink{/c/3/total}{p=7&q=0.5&c=4096&n=1}{Try the interactive Total VRAM calculator}
...
Numerical precision determines bytes per parameter. Lower precision means smaller models but potentially reduced quality. The trade-off is usually worthwhile for inference because: (1) inference doesn't accumulate errors like training does, (2) modern quantization methods preserve critical weights at higher precision, and (3) the quality loss is often imperceptible for practical applications.
...
\calclink{/c/3/breakeven}{hw=2400&setup=0&rent=0.44&hours=720&power=0.15&watts=300}{Try the interactive Break-Even calculator}
...
With hardware requirements understood, the next question is: which models fit your hardware? Chapter~\ref{ch:models} covers model selection---navigating the landscape of available models, understanding format differences (GGUF, GPTQ, AWQ), and choosing quantization levels that balance quality against your VRAM constraints.
...
\input{chapters/chapter03/references}
